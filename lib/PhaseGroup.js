...
class PhaseGroup extends EventEmitter{
    ...
    async load(){
        ...
        try{
            ...
            let cacheKey = format('phasegroup::%s::%s', this.id, this.expandsString);
    ...
    async getPlayers(fromCacheTF){
        ...
        try {
            let cacheKey = format('phaseproup::%s::players', this.id);
            if (fromCacheTF) {
                let cached = await Cache.get(cacheKey);
                if (cached) {
                    this.players = cached;
    ...
    getPhaseId(){
        if(this.data)
            return this.data.entities.groups.phaseId || this.nullValueString('Phase Id');
        else throw new Error('No data to get PhaseGroup property Phase Id');
    }
