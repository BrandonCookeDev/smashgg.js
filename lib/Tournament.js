...
class Tournament extends EventEmitter{
	...
	constructor(tournamentId, options={}){
		...
		// set properties
		this.data = {};
		this.name = isNaN(parseInt(tournamentId)) ? tournamentId : parseInt(tournamentId);
		this.isCached = isCached;
		this.rawEncoding = LEGAL_ENCODINGS.includes(options.rawEncoding) ? options.rawEncoding : DEFAULT_ENCODING;
	...
	// Convenience methods
	static getTournament(tournamentName, expands=undefined, isCached=true){
		let deferred = when.defer();
		try{
			let T = new Tournament(tournamentName, expands, isCached);
			T.on('ready', function(){
				deferred.resolve(T);
			});
	...
	static getTournamentById(tournamentId, expands=undefined, isCached=true){
		let deferred = when.defer();
		try{
			parseInt(tournamentId);
			Tournament.getTournamentById(tournamentId, expands, isCached)
	...
	async getAllPlayers(fromCacheTF){
		...
		try{
			...
			let allPlayers = await Promise.all(
	...
	async getAllEvents(fromCacheTF){
		...
		try{
			...
			let allEvents = await Promise.all(
				this.getData().entities.event.map(async (event) => {
