import {IStreamData, IStreamDataFull} from '../../lib/interfaces/IStream'

export const stream1: IStreamData = {
	id: 10493,
	eventId: null,
	tournamentId: 6620,
	streamName: 'recursiongg',
	numSetups: null,
	streamSource: 'TWITCH',
	streamType: 1,
	streamTypeId: null,
	isOnline: true,
	enabled: true,
	followerCount: 9609,
	removesTasks: true,
	streamStatus: 'Tipped Off 12 Top 8 ft M2K, Hax$, n0ne, Swedish, Cal, Colbol, Dizzkid, iBDW',
	streamGame: 'Super Smash Bros. Melee',
	streamLogo: 
		'https://static-cdn.jtvnw.net/jtv_user_pictures/recursiongg-profile_image-30c27f4bc6a59cc8-300x300.png'
}

export const stream2: IStreamData = {
	id: 574,
	eventId: null,
	tournamentId: 1609,
	streamName: 'vgbootcamp',
	numSetups: null,
	streamSource: 'TWITCH',
	streamType: 1,
	streamTypeId: null,
	isOnline: null,
	enabled: true,
	followerCount: null,
	removesTasks: true,
	streamStatus: null,
	streamGame: null,
	streamLogo: null
}

export const stream3: IStreamData = {
	id: 40210,
	eventId: null,
	tournamentId: 36179,
	streamName: 'capcomfighters',
	numSetups: null,
	streamSource: 'TWITCH',
	streamType: 1,
	streamTypeId: null,
	isOnline: false,
	enabled: true,
	followerCount: 294404,
	removesTasks: null,
	streamStatus: 'EVO 2018 - Day 2',
	streamGame: 'Street Fighter V',
	streamLogo: 
		'https://static-cdn.jtvnw.net/jtv_user_pictures/daebf3b9-203e-4c1c-9ac5-0e8bf445be86-profile_image-300x300.png'
}

export const streamData1: IStreamDataFull = {
	stream: stream1
}

export const streamData2: IStreamDataFull = {
	stream: stream2
}

export const streamData3: IStreamDataFull = {
	stream: stream3
}
