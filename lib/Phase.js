...
class Phase extends EventEmitter{
    ...
    async load(){
        ...
        try{
            ...
            let cacheKey = format('phase::%s::%s', this.id, this.expandsString);
    ...
    getName(){
        if(this.data)
            return this.data.entities.phase.name || this.nullValueString('Name');
        throw new Error('No data to get Phase property Name');
    }

    getEventId(){
        if(this.data)
            return this.data.entities.phase.eventId || this.nullValueString('Event Id');
