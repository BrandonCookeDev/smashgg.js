...
class Tournament extends EventEmitter{
    ...
    constructor(tournamentId, expands, isCached){
        super();

        if(!tournamentId)
            throw new Error('Tournament Name cannot be null');

        // if we can parse, it's an ID number
    ...
    async getAllPlayers(fromCacheTF){
        ...
        try{
            ...
            let allPlayers = await Promise.all(
                this.data.entities.groups.map(async (group) => {
    ...
    async getAllEvents(fromCacheTF){
        ...
        try{
            ...
            let allEvents = await Promise.all(
                this.data.entities.event.map(async (event) => {
