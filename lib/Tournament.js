...
class Tournament extends EventEmitter{
	...
	async getAllPlayers(options={}){
		...
		try{
			...
			let players = data.tournament.participants.nodes;
	...
	async getAllSets(options={}){
		...
		try{
			...
			let results = await Promise.all(pgIds.map(async (id) => await NI.query(graphQueries.phaseGroupSets, {id: id})));
	...
	async getIncompleteSets(options={}){
		...
		try{
			...
			let incompleteSets = sets.filter(set => set.completedAt == undefined);
	...
	async getCompleteSets(options={}){
		...
		try{
			let sets = await this.getAllSets(options);
			let completedSets = sets.filter(set => set.completedAt != undefined);
	...
	async getSetsXMinutesBack(minutesBack, options={}){
		...
		try{
			...
			let setsXMinutesBack = sets.filter(set => set.completedAt && set.completedAt < timestamp);
