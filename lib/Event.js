...
class Event extends EventEmitter{
    ...
    async getEventPhases(fromCacheTF){
        ...
        try{
            log.info('Getting Phases for Event ' + this.tournamentSlug);
            let cacheKey = format('event::%s::phases', this.tournamentSlug);
            if(fromCacheTF){
                let cached = await Cache.get(cacheKey);
                if(cached){
                    this.phases = cached;
    ...
    async getEventPhaseGroups(fromCacheTF){
        ...
        try{
            log.info('Getting Phase Groups for Event ' + this.tournamentSlug);
            let cacheKey = format('event::%s::groups', this.tournamentSlug);
            if(fromCacheTF){
                let cached = await Cache.get(cacheKey);
                if(cached){
