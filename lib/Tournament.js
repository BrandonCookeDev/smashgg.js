...
class Tournament extends EventEmitter{
    ...
    constructor(tournamentName, expands, isCached){
        ...
        this.url = format(TOURNAMENT_URL, this.slug, this.expandsString);
    ...
    async load(){
        ...
        try{
            ...
            let cacheKey = format('tournament::%s', this.slug);
    ...
    async getAllPlayers(fromCacheTF){
        ...
        try{
            log.info('Gettings players for ' + this.getName());
            let cacheKey = format('tournament::%s::players', this.slug);
            if(fromCacheTF){
                let cached = await Cache.get(cacheKey);
                if(cached){
                    this.players = cached;
    ...
    async getAllSets(fromCacheTF){
        ...
        try{
            ...
            let cacheKey = format('tournament::%s::sets', this.slug);
            if(fromCacheTF){
                let cached = await Cache.get(cacheKey);
                if(cached){
                    this.players = cached;
    ...
    async getAllEvents(fromCacheTF){
        ...
        try{
            ...
            let cacheKey = format('tournament::%s::events', this.slug);
            if(fromCacheTF){
                let cached = await Cache.get(cacheKey);
                if(cached){
                    this.players = cached;
            ...
            for(let i in this.data.entities.event){
                ...
                let p = new Promise(function(resolve, reject) {
                    ...
                    let E = new Event(ThisTournament.slug, eventName);
                    E.on('ready', function() {
                        resolve(E);
                    });
                });
