...
class Tournament extends EventEmitter{
	...
	async getAllSets(options={}){
		...
		try{
			...
			let sets = await Promise.all(pgIds.map(async (id) => await NI.query(graphQueries.phaseGroupSets, {id: id})));
			return sets;
	...
	async getAllEvents(options={}){
		...
		try{
			...
			let events = eventData.map(event => {
				let data = {};
				let E = new Event(event.slug, tournamentData.name, {noLoad: true, rawEncoding: rawEncoding, isCached: isCached});
