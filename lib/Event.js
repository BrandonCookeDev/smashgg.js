...
class Event extends EventEmitter{
	...
	// Methods
	async get(){
		let data = await NI.query(graphQueries.event);
		let identifier = isNaN(this.eventId) ? 'name' : 'id'; 
		let eventData = data.tournament.events.filter(event => event[identifier] == this.eventId);
		data.tournament.events = eventData; // overwrite the array of events. In the future we should cache these
		return data;
	}
	...
	getFromEventData(prop){
		...
		if(data && data.tournament && data.tournament.events) {
			if (!data.tournament.event[prop])
				log.error(this.nullValueString(prop));
			return data.tournament.event[prop];
		}
		else{
			log.error('No data to get Tournament property Id');
			return null;
	...
	getTournamentId(){
		return this.getFromEventData('tournamentId');
	}
